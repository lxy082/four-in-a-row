...
## 学习型对手画像（本地）
学习数据仅保存在浏览器本地（localStorage），不会上传任何服务器。

### 模块组成
1. **偏好画像**（`g4_profile_v1`）：统计玩家对 25 根柱子的热度、中心/边缘偏好、平均落点高度、连续叠高等指标。
2. **经验库**（`g4_memory_v1`）：对局结束后回溯最近 10 个关键局面，记录 AI 当时走法的胜负统计，命中时优先挑选高分招。
3. **权重微调**（`g4_weights_v1`）：每局结束后微调评估权重（防守与进攻），并限制范围，避免过拟合。

### AI 使用方式
- 画像用于 move ordering（中心优先 + 对手高频柱优先拦截）。
- 经验库命中后优先选择高分招，再加入可控随机性。
- 学习模式可在控制面板中开关，并提供清空学习/经验/权重的按钮。

## 威胁检测优先级
AI 决策顺序：
1. **AI 立即胜**（一步成 4）
2. **对手立即胜**（必须堵）
3. **高危威胁**：对手开放三连 / 双威胁（必须优先堵）
4. **制造高危**：AI 自己的开放三连/双威胁
5. **常规搜索 + 评估 + 随机采样**

威胁检测会考虑重力约束（只有当前可落的空位才算威胁落点）。
...
- 对开放两连/三连增加强惩罚/强奖励，提升防守。

### 难度说明
- 仅保留一个“最高可用强度”难度，确保 AI 在 30 秒硬上限内完成落子。

### AI 超时与卡顿修复说明
- **硬上限**：AI 必须在 30 秒内完成思考并落子，超过即 fallback 选择合法步。
- **原因**：此前 AI 搜索没有严格的“硬上限”，可能在深度递归中耗时过长，导致 UI 看起来卡住。
- **修复**：主线程在每回合为 AI 计算设置时间预算（与计时模式联动），AI 搜索在 worker 内部按 `deadline` 逐层迭代并在超过时间上限时立即返回当前最佳着法。
